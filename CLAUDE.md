...
| `linear_optimizer.sh` | Linear task cache-locality optimizer (LINOPT-2 / TAP-591). `linear_optimizer_run` fires at session start (background) when `RALPH_TASK_SOURCE=linear` and `LINEAR_API_KEY` is set. Fetches top-N open issues, scores by Jaccard(last_completed_files, issue_body_paths) + shared-parent-dir bonus, uses ralph-explorer (Haiku) as fallback for top-3 priority issues with no body paths (results cached in `.ralph/.linear_optimizer_cache.json`). Atomic write of best candidate ID to `.ralph/.linear_next_issue` for `build_loop_context()` to consume. Guards: `RALPH_NO_LINEAR_OPTIMIZE=true`, push-mode (no API key), missing project config. |
| `linear_backend.sh` | Linear API task backend — `linear_get_open_count`, `linear_get_done_count`, `linear_get_next_task`, `linear_check_configured`. Used when `RALPH_TASK_SOURCE=linear`. **Fail-loud (TAP-536)**: on API/network/parse errors the count/task functions print **nothing to stdout** and return non-zero, with a single structured `linear_api_error: op=<name> reason=<timeout\|network\|http_NNN\|graphql_errors\|parse\|...>` line on stderr (no secrets). Callers MUST distinguish "exit non-zero" (unknown) from "exit 0 + value" (real result) so a transient API outage cannot trip a false `plan_complete` exit. **Push-mode (TAP-741)**: when `LINEAR_API_KEY` is unset (OAuth-via-MCP deployments), the count functions fall back to reading `linear_open_count` / `linear_done_count` from `.ralph/status.json`, written by the on-stop hook from Claude's RALPH_STATUS block. Entries older than `RALPH_LINEAR_COUNTS_MAX_AGE_SECONDS` (default 900) abstain via the same TAP-536 path. Iteration 1 has no hook write yet and abstains (logged INFO, not WARN); iteration 2+ reads fresh counts. `linear_get_next_task` still requires the API key — Claude picks its own next task via the Linear MCP in push-mode. `linear_check_configured` now requires only `RALPH_LINEAR_PROJECT` (API key is optional). |
