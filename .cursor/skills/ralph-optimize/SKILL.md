...
description: >
  ...
  resolves vague tasks via exploration, and reorders for dependency order,
...
If the user's message includes `--force`, force a full import-graph rebuild. If it includes `--no-explorer`, skip the vague-task resolution step.
...
2. Rebuild the import graph (force rebuild when `--force` is requested):

   ```bash
   import_graph_ensure "." ".ralph/.import_graph.json"
   ```

3. Resolve vague tasks (skip when `--no-explorer` is requested):
